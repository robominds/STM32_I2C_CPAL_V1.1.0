...
static void IOE_INT_GPIO_Config(void);
...
{  
  ...
  tmp = IOE_Buffer[0];
  IOE_Buffer[0] = IOE_Buffer[1];
  IOE_Buffer[1] = tmp;
  
  /* return buffer value */
  return *(uint16_t *)IOE_Buffer; 
}
...
{   
  ...
  IOE_DevStructure.wCPAL_Options  = CPAL_OPT_DMATX_TCIT | CPAL_OPT_DMARX_TCIT ;
#endif /* IOE_IT */
...
/**
  * @brief  Initialize the GPIO pins used by the IO expander.
  * @param  None
  * @retval None
  */
static void IOE_INT_GPIO_Config(void)
{
  ...
  /* Enable IOE_INT_PIN Port clock */
  RCC_APB2PeriphClockCmd(IOE_IT_GPIO_CLK , ENABLE);
